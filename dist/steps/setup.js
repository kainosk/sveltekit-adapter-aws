...
export const setup = async ({ builder, tmp, options }) => {
    const { appDir, paths: { base } } = builder.config.kit;
    ...
    builder.copy(path.resolve(cdkPath, 'cdk.json'), path.join(options.out, 'cdk.json'));
    builder.copy(path.resolve(cdkPath, 'package.json'), path.join(options.out, 'package.json'));
    builder.copy(path.resolve(cdkPath, 'mock', 'synth.ts'), path.join(options.out, 'bin', 'synth.ts'), {
        replace: {
    ...
    await copy(path.join(cdkPath, 'external', 'params.ts'), path.join(options.out, 'external', 'params.ts'), {
    ...
    await copy(path.join(cdkPath, 'external', 'cdk-modifiers.ts'), path.join(options.out, 'external', 'cdk-modifiers.ts'), {
        '/* $$__ADAPTER_IMPORTS__$$ */': options.adapterImports?.join('\n') ?? '',
        '(lambdaFunction: cdk.aws_lambda.Function) => {} /* $$__LAMBDA_MODIFIER__$$ */': options.lambdaModifier?.toString() ??
            '(lambdaFunction: cdk.aws_lambda.Function) => {}'
    });
    console.log('options.architecture', options.architecture);
    builder.copy(path.join(root, 'cdk', 'arch', `${options.architecture}.ts`), path.join(options.out, 'bin', 'cdk-stack.ts'));
    builder.writeServer(tmp);
    await writeFile(path.join(tmp, 'manifest.js'), `export const manifest = ${builder.generateManifest({
        relativePath: './'
    })};\n\nexport const prerendered = new Set(${JSON.stringify(builder.prerendered.paths)});\n`);
    builder.copy(path.join(root, 'cdk', 'external', 'cf2.js'), path.join(options.out, 'cf2', 'index.js'), {
        replace: {
            __DOMAIN_NAME__: options.domain?.fqdn ?? ''
        }
    });
